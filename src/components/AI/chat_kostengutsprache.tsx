    ...
    initialMessages: [
      {
        ...
        role: 'system',
        content: `Du bist ein KI-Modell, das die Kostengutsprache für medizinische Leistungen oder Medikamente erstellt.
        Der Input gibt dir Informationen ueber die gewuenschte Gutspracheart und bekannte Diagnosen.
        Orientiere dich an folgenden Format und formuliere [...] aus. Ergänze eine Begründung und nutze das Format, wie in einem medizinischen Brief an eine Krankenkasse.

        Sehr geehrte Damen und Herren,
        Wir bitten um Kostengutsprache für [...], welches bei dem Patienten aus medizinischer Sicht sehr villvoll und hilfreich ist.
        [Begründung 1]
        [Begründung 2]
        [Vorteil, wenn die ersuchte Leistung durchgeführt wird]
      ...
      <div className='container'>
        <div className='mt-3 w-full max-w-full text-left relative -ml-4'>
          <h1 className='font-medium pt-5 text-zinc-900 dark:text-zinc-100'>
            Closed-Beta Test: Gutspracheart und Diagnose eingeben ➜ Formulierungsvorschlag für eine Kostengutsprache erhaltenen
          </h1>
        </div>
          ...
          <div className='mt-3 w-3/4 text-left relative -ml-4'>
            ...
            {showFollowUpButtons && (
              <div className='flex justify-start mb-3 space-x-3'>
                <button
                  ...
                  onClick={() => setInput('Weitere Diagnose hinzufügen')}
                >
                  <FaLightbulb className='text-yellow-500' />
                  <span>Weitere Diagnose hinzufügen</span>
                </button>
                <button
                  className='bg-gray-200 text-black px-4 py-2 rounded flex items-center space-x-2'
                  onClick={() => setInput('Weitere Begründung hinzufügen')}
                >
                  <FaLightbulb className='text-yellow-500' />
                  <span>Weitere Begründung hinzufügen</span>
                </button>
                <button
                  className='bg-gray-200 text-black px-4 py-2 rounded flex items-center space-x-2'
                  onClick={() => setInput('Vorteile hervorheben')}
                >
                  <FaLightbulb className='text-yellow-500' />
                  <span>Vorteile hervorheben</span>
                </button>
              </div>
            )}
            ...
            <form onSubmit={onSubmit} className='relative'>
              <Input
                ...
                placeholder='Gutspracheart + Diagnose nennen (Bsp: LZ-Physio, Cerebralem Insult L)'
          ...
          <div className='mt-1 w-1/4 text-left relative pl-5'>
            <div className='mt-2 bg-gray-100 dark:bg-gray-800 pl-13 rounded-md max-h-[500px] overflow-y-auto'>
              ...
              <ul className='p-1 mt-1 text-sm text-zinc-700 dark:text-zinc-300'>
                {[
                  'Medi Rybelsus, Diabetis',
                  'Ernaehrungsberatung, Adipositas',
                  'LZ-Physio, Cerebralem Insult L',
                  'Psychotherapie, Depression',
                  'Rehabilitation, Rückenmarksverletzung',
                  'Ergotherapie, Schlaganfall',
                  'Hörgerät, Schwerhörigkeit',
                  'Diätberatung, Fettleibigkeit'
                ].map((example, index) => (
                  <li key={index} onClick={() => handleLiClick(example)} className='cursor-pointer'>{example}</li>
                ))}
              </ul>
              <p className='p-1 font-semibold text-zinc-900 dark:text-zinc-100 mt-3'>Wichtigste Abkürzungen:</p>
              <ul className='p-1 mt-1 text-sm text-zinc-700 dark:text-zinc-300'>
                <li>LZ: Langzeit</li>
                <li>Physio: Physiotherapie</li>
                <li>PT: Physiotherapie</li>
                <li>L: Links</li>
                <li>R: Rechts</li>
              </ul>
            </div>
