...
export default function Chat_medis() {
    ...
    initialMessages: [
      {
        ...
        role: 'system',
        content: `Du bist ein KI-Modell, das Medikamente und deren Wirkung kennt. Du kannst Medikamente vergleichen, gibst Auskunft
        über Wirkstoffe und deren Nebenwirkungen, sowie Interaktionen, GFR, Pharmacode und GTIN. Wenn möglich, gibst du auch Hinweise auf Alternativen und erstellen 
        Tabellen zur Übersicht. Erstelle zudem auch ein Dosierungsvorschlag in Abhängigkeit von Alter, Geschlecht und Gewicht.

        Dein Publikum sind Hausaerzte, welche Medikamente abgeben und Patienten beraten. Keine Einleitung, kein Schlusssatz. Hohe Informationsdichte.
  ...
  // Show follow-up buttons when the assistant sends a message
  useEffect(() => {
      ...
      <div className='container'>
        <div className='mt-3 w-full max-w-full text-left relative -ml-4'>
          <h1 className='font-medium pt-5 text-zinc-900 dark:text-zinc-100'>
            Closed-Beta Test: Medikament (optional: + Kontext) ➜ Infos zur Unverträglichkeiten/Nebenwirkungen.
          </h1>
        </div>
          ...
          <div className='mt-3 w-3/4 text-left relative -ml-4'>
            ...
            {showFollowUpButtons && (
              <div className='flex justify-start mb-3 space-x-3'>
                <button
                  ...
                  onClick={() => setInput('Vergleich mit einem anderen Medikament')}
                >
                  <FaLightbulb className='text-yellow-500' />
                  <span>Vergleich mit einem anderen Medikament</span>
                </button>
                <button
                  className='bg-gray-200 text-black px-4 py-2 rounded flex items-center space-x-2'
                  onClick={() => setInput('Nebenwirkungen')}
                >
                  <FaLightbulb className='text-yellow-500' />
                ...
                <button
                  className='bg-gray-200 text-black px-4 py-2 rounded flex items-center space-x-2'
                  onClick={() => setInput('Dosierungsvorschlag')}
                >
                  <FaLightbulb className='text-yellow-500' />
                  <span>Dosierungsvorschlag</span>
                </button>
                <button
                  className='bg-gray-200 text-black px-4 py-2 rounded flex items-center space-x-2'
                  onClick={() => setInput('mehr Details zu GFR bei diesem Medikaent')}
                >
                  <FaLightbulb className='text-yellow-500' />
                  <span>GFR</span>
                </button>
            ...
            <form onSubmit={onSubmit} className='relative'>
              <Input
                ...
                placeholder='Medikament und optional: + Kontext (Beispiel Ramipril, Diabetiker T2). Folgefragen möglich.'
          ...
          <div className='mt-1 w-1/4 text-left relative pl-5'>
            <div className='mt-2 bg-gray-100 dark:bg-gray-800 pl-13 rounded-md max-h-[500px] overflow-y-auto'>
              ...
              <ul className='p-1 mt-1 text-sm text-zinc-700 dark:text-zinc-300'>
                {[
                  'Ramipril, Diabetiker T2',
                  'arlevert, schwindel',
                  'Aspirin, Blutdruck hoch, 55 J, w',
                  'Ibuprofen, Magenprobleme, 40 J, m',
                  'Metformin, Gewichtsverlust, 60 J, m',
                  'Paracetamol, Fieber, 35 J, w',
                  'Simvastatin, Cholesterin hoch, 50 J, m',
                  'Amoxicillin, Infektion, 30 J, 92kg, bakt.',
                  'Losartan, Bluthochdruck, 65 J, m'
                ].map((example, index) => (
              ...
              <ul className='p-1 mt-1 text-sm text-zinc-700 dark:text-zinc-300'>
                ...
                <li>GFR: Glomeruläre Filtrationsrate</li>
                <li>GTIN: Global Trade Item Number</li>
                <li>Pharmacode: Pharmazeutischer Code</li>
              </ul>
            </div>
            <div className='mt-3'>
              <button
